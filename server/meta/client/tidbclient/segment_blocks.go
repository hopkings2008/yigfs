...
func(t *TidbClient) GetIncompleteUploadSegs(ctx context.Context, segs []*types.UpdateSegBlockInfo) (segsResp *types.GetIncompleteUploadSegsResp, err error) {
	segsResp = &types.GetIncompleteUploadSegsResp{}
	sqltext := "select seg_end_addr from segment_blocks where seg_id0=? and seg_id1=? and is_deleted=? order by seg_end_addr desc limit 1;"
	var stmt *sql.Stmt
	stmt, err = t.Client.Prepare(sqltext)
	if err != nil {
		helper.Logger.Error(ctx, fmt.Sprintf("GetIncompleteUploadSegs: failed to prepare %s, err: %v", sqltext, err))
		err = ErrYIgFsInternalErr
		return
	}
	defer func() {
		err = stmt.Close()
		if err != nil {
			helper.Logger.Error(ctx, fmt.Sprintf("GetIncompleteUploadSegs: failed to close stmt for %s, err: %v", sqltext, err))
		}
	}()
	
	var max_addr int64
	for _, seg := range segs {
		row := t.Client.QueryRow(sqltext, seg.SegmentId0, seg.SegmentId1, types.NotDeleted)
		err = row.Scan(
			&max_addr,)
		if err == sql.ErrNoRows {
			err = nil
			...
			helper.Logger.Info(ctx, fmt.Sprintf("Failed to get the block's max end_addr, seg_id0: %d, seg_id1: %v", seg.SegmentId0, seg.SegmentId1))
			err = ErrYIgFsInternalErr
			return
		}

		if seg.LatestOffset < int(max_addr) {
	...
	helper.Logger.Info(ctx, fmt.Sprintf("Succeed to get incomplete upload segs, segs number: %v", len(segsResp.Segments)))
	return
}